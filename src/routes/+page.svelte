<script lang="ts">
	...
	function parseExecuteCommandTags(content: string): string[] {
	...
	async function executeBridgeCommand(chatId: string, commandText: string) {
		const conv = conversations.find(c => c.id === chatId);
		const project = conv?.projectId ? projects.find(p => p.id === conv.projectId) : null;
		if (!project || !project.localPath || !enableWorkspaceBridge || !workspaceBridgeUrl) {
			return;
		}

		// Append a temporary status message to show execution starting
		const statusMsgId = `msg-${Date.now()}-system-status`;
		const statusMessage: Message = {
			id: statusMsgId,
			role: 'system',
			content: `⚙️ กำลังรันคำสั่ง \`${commandText}\`... (กรุณายืนยันการรันใน Terminal ของ mcp-bridge)`,
			timestamp: Date.now()
		};

		conversations = conversations.map((c) => {
			if (c.id === chatId) {
				return { ...c, messages: [...c.messages, statusMessage] };
			}
			return c;
		});
		...
		try {
			const cleanUrl = workspaceBridgeUrl.replace(/\/$/, '');
			const res = await fetch(`${cleanUrl}/execute`, {
				method: 'POST',
				headers: {
					'Content-Type': 'application/json',
					'x-local-path': project.localPath
			...
			if (!res.ok) {
				...
				throw new Error(parsedError?.error || errorText || `HTTP error ${res.status}`);
			}

			const data = await res.json();
			const output = data.output || '(ไม่มีผลลัพธ์)';

			// Remove status message and add output message
			const outputMessage: Message = {
				id: `msg-${Date.now()}-user`,
				role: 'user',
				content: `[ระบบรันคำสั่งสำเร็จ]\nคำสั่ง: \`${commandText}\`\nผลลัพธ์:\n\`\`\`\n${output}\n\`\`\`\nกรุณาดำเนินการขั้นตอนถัดไปต่อเลยครับ`,
				timestamp: Date.now()
			};

			conversations = conversations.map((c) => {
				if (c.id === chatId) {
					// Filter out the status message and append the output
					const filteredMessages = c.messages.filter(m => m.id !== statusMsgId);
					return { ...c, messages: [...filteredMessages, outputMessage] };
				}
				return c;
			});

			// Now, trigger the AI response loop with the updated messages!
			isGenerating = true;
			...
			if (activeConv) {
				...
				const systemPrompt = await getCombinedSystemPrompt(activeConv);
			...
			console.error('Failed to execute command via bridge:', e);
			const errorMessage: Message = {
				id: `msg-${Date.now()}-user`,
				role: 'user',
				content: `[ระบบรันคำสั่งล้มเหลว]\nคำสั่ง: \`${commandText}\`\nข้อผิดพลาด:\n\`\`\`\n${e.message || String(e)}\n\`\`\``,
				timestamp: Date.now()
			};
			...
			// Also trigger AI to respond/recover from failure!
			isGenerating = true;
			abortController = new AbortController();
	...
	async function getCombinedSystemPrompt(conv: Conversation | null): Promise<string> {
		...
		let canvasFiles: any[] = [];
		try {
			canvasFiles = await db.canvasFiles.where({ chatId: conv.id }).toArray();
		} catch (e) {
	...
	async function executeModelChain(
		...
		try {
			...
			// Check and execute command tags if in project-linked chat and workspace bridge is enabled
			if (finalMsg && finalMsg.content && enableWorkspaceBridge && activeConv?.projectId) {
				const commandsToRun = parseExecuteCommandTags(finalMsg.content);
				if (commandsToRun.length > 0) {
					// Execute the first command (further commands can be requested in subsequent turns)
					executeBridgeCommand(activeConvId, commandsToRun[0]);
				}
			}
		} catch (error: any) {
